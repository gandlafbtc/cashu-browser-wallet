...
import { messagesStore } from '$lib/stores/persistent/message.js';
import { type Message } from '$lib/db/models/types.js';
import type { ProfilePointer } from 'nostr-tools/nip19';
import { relaysStore } from '$lib/stores/persistent/relays.js';
import { wordlist } from '@scure/bip39/wordlists/english';
import { discoveredContacts } from '$lib/stores/session/contactdiscover.js';
...
import { receiveEcash } from './actions.js';
import { nostrPool } from '$lib/stores/session/nostr.js';
import { discoveredMints } from '$lib/stores/session/mintdiscover.js';


export const createAlias = () => {
	const wordlistLength = wordlist.length;
	return (
		wordlist[Math.floor(Math.random() * wordlistLength)] +
		' ' +
		wordlist[Math.floor(Math.random() * wordlistLength)]
	);
};
...
export const discoverMints = async () => {
	discoveredMints.set([]);
	const activeRelays = get(relaysStore)
		.filter((r) => r.isOn)
		.map((r) => r.url);
...
const sendNip17DirectMessage = async function (
	...
	const ndk = new NDK({
		explicitRelayUrls: [...activeRelays, ...(relays ?? [])],
	...
	sealEventReceiver.content = nip44.v2.encrypt(
		dmEventString,
		nip44.v2.utils.getConversationKey(seedSignerSecKey, receiverPubkey)
	);
	...
	sealEventSender.content = nip44.v2.encrypt(
		dmEventString,
		nip44.v2.utils.getConversationKey(seedSignerSecKey, signerPubKey)
	);
	...
	wrapEventReceiver.content = nip44.v2.encrypt(
		sealEventString,
		nip44.v2.utils.getConversationKey(randomPrivateKey, receiverPubkey)
	);
	...
	wrapEventSender.content = nip44.v2.encrypt(
		sealEventSenderString,
		nip44.v2.utils.getConversationKey(randomPrivateKey, signerPubKey)
	);
		...
		onevent: (wrapEvent: Event) => {
			try {
				...
				const dmEventString = nip44.v2.decrypt(
					sealEvent.content,
					nip44.v2.utils.getConversationKey(seedSignerSecKey, sealEvent.pubkey)
				);
				...
				if (
					token &&
					get(settings)[0].tokens.autoReceive &&
					get(mints).find((m) => m.url === token.mint)
				) {
					receiveEcash(token);
				}
